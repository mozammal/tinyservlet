...
public class JspToServletConverter {
  ...
  private static final String PACKAGE_NAME_TRANSLATED_SERVLET =
      "org.tinywebserver.servlet.translated.";
  ...
  private final String DESTINATION_DIRECTORY_CONVERTED_SERVLET =
      TinyServletConfig.getTinyServletConfigInstance()
          .getProperties()
          .getProperty("tinywebserver.jsp.destination.directory");

  private final String JSP_ROOT_DIRECTORY =
      TinyServletConfig.getTinyServletConfigInstance()
          .getProperties()
          .getProperty("tinywebserver.jsp.directory");
  ...
  public void traverseJspFolderAndConvertToJsp(File curDir)
      throws IOException, InterruptedException, ClassNotFoundException {

    File[] filesList = curDir.listFiles();
    for (File file : filesList) {
      if (file.isDirectory()) traverseJspFolderAndConvertToJsp(file);
      if (file.isFile()) {
        if (isUpdateServletConfiguration()) updateServletConfiguration(file);
        else translateJspToServlet(file);
      }
    }
  }
  ...
  private void updateServletConfiguration(File file) throws ClassNotFoundException, IOException {
    ...
    setJavaFileName(
        TinyWebServerUtility.capitalizeFirstLetter(
            file.getName().replace(JSP_FILE_EXTENSION, JAVA_FILE_EXTENSION)));
    ...
    String servletName =
        PACKAGE_NAME_TRANSLATED_SERVLET + getJavaFileName().replace(JAVA_FILE_EXTENSION, "");
    Class classInstance = Class.forName(servletName);
    tinyServletConfigInstance.getTinyServletConfigs().putIfAbsent(resourceLocation, classInstance);
  }

  private void createPrintWriter(File file)
    ...
    Writer writer =
        new BufferedWriter(
            new OutputStreamWriter(new FileOutputStream(fileNameJavaWithFullPath, false), UTF_8));
    PrintWriter printWriter = new PrintWriter(writer, true);
    setWriter(printWriter);
  }

  public void translateJspToServlet(File file)
      throws IOException, InterruptedException, ClassNotFoundException {
    ...
    if (destinationDirectory == null) setDestinationDirectory(getDestinationDirectoryServlet());
    createPrintWriter(file);
    String className =
        TinyWebServerUtility.capitalizeFirstLetter(file.getName().replace(JSP_FILE_EXTENSION, ""));
  ...
  private void createServlet() {
    ...
    writer.println(
        "public class " + getJavaFileName().replace(".java", "") + " extends TinyServlet { ");
    writer.println("");
  }

  private void constructServlet(BufferedReader bufferedReader, String fileName)
  ...
  private void addScriptletHtmlExpression() {
    ...
    for (Token token : getTokens()) {
      ...
      if (tokenType == JspTokenType.HTML)
        tokenValue =
            token.getValue().replace("\\", "\\\\").replace("\"", "\\\"").replace("'", "\\'");
      switch (tokenType) {
        case HTML:
