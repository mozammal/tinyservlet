...
public class JspToServletProcessor {
  ...
  public void translateJspToServlet(boolean updateServletConfiguration)
      throws IOException, InterruptedException, ClassNotFoundException, URISyntaxException {

    TinyServletConfig tinyServletConfigInstance = TinyServletConfig.getTinyServletConfigInstance();
    JspToServletConverter jspToServletConverter =
        new JspToServletConverter(updateServletConfiguration);
    String jspDirectory =
        tinyServletConfigInstance.getProperties().getProperty("tinywebserver.jsp.directory");
    ClassLoader classloader = Thread.currentThread().getContextClassLoader();
    URL jspResourceDirectory = classloader.getResource(jspDirectory);

    if (jspResourceDirectory == null) return;
