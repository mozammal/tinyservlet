...
public class HttpProcessor extends BaseHttpProcessor implements Runnable {
  ...
  public void run() {
    ...
    try {
      ...
      TinyServlet matchingTinyServlet =
          TinyServletConfig.getTinyServletConfigInstance()
              .getMatchingTinyServlet(httpTinyServletRequest.getPath());

      if (matchingTinyServlet != null) {
        TinyServletHandler tinyServletHandler = new TinyServletHandler();
        tinyServletHandler.processTinyServlet(
            matchingTinyServlet, httpTinyServletRequest, httpTinyServletResponse);
      } else {
